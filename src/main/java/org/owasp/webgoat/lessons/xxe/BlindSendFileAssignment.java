...
import org.apache.commons.io.FilenameUtils; // Added for FilenameUtils
...
public class BlindSendFileAssignment implements AssignmentEndpoint, Initializable {
  ...
  private void createSecretFileWithRandomContents(WebGoatUser user) {
    ...
    // Remediation: Sanitize username for path construction to prevent path traversal
    ...
    Path expectedBasePath = new File(webGoatHomeDirectory, "/XXE/").toPath().normalize();
    Path resolvedPath = targetDirectory.toPath().normalize();
    if (!resolvedPath.startsWith(expectedBasePath)) {
        log.error("Path traversal attempt detected for user: {}", user.getUsername());
        return; // Abort file creation if path traversal is detected
    }
    ...
    try {
      ...
      log.error("Unable to write 'secret.txt' to '{}' for user '{}'", targetDirectory, user.getUsername());
    }
  }
