    ...
    try (var connection = dataSource.getConnection()) {
      PreparedStatement statement =
          connection.prepareStatement(
              "select password from challenge_users where userid = ? and password = ?"); // Using parameterized query
      statement.setString(1, username_login); // Binding username parameter
      statement.setString(2, password_login); // Binding password parameter
