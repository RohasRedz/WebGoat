...
public class Assignment5 implements AssignmentEndpoint {
  ...
  public AttackResult login(
    ...
    try (var connection = dataSource.getConnection()) {
      // Remediation: Use PreparedStatement with parameterized queries to prevent SQL Injection
      PreparedStatement statement =
          connection.prepareStatement(
              "select password from challenge_users where userid = ? and password = ?");
      statement.setString(1, username_login); // Bind username_login parameter
      statement.setString(2, password_login); // Bind password_login parameter
